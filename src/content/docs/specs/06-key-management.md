---
title: "Spec: Key Management"
description: "Key generation, storage, rotation, and recovery for PDTF 2.0 credential issuers and holders."
---
...
- Encryption keys (PDTF 2.0 credentials are signed, not encrypted)
...
### 3.4 Status List Signing Keys

| Property | Value |
|----------|-------|
| **Purpose** | Sign Bitstring Status List credentials for revocation |
| **DID method** | Same as the credential issuer |
| **Cardinality** | Typically the same key as the issuer's adapter/platform key |
| **Storage** | Google Cloud KMS |
| **Rotation** | Follows the parent key's rotation |

Status List credentials are themselves VCs — they must be signed. In most cases, the same key that signs a credential also signs the Status List that can revoke it. This simplifies key management without reducing security: the issuer who can create a credential should also be able to revoke it.

**Exception:** If a separate revocation authority is introduced (e.g., a regulatory body that can revoke credentials it didn't issue), it would need its own signing key and TIR entry.
...
│ Status List Keys │ (same as     │ KMS       │ Bitstring Status     │
│                  │  issuer)     │           │ List VCs             │
└──────────────────┴──────────────┴───────────┴──────────────────────┘
```
...
│   │
│   └── Key Ring: status-lists
│       └── Key: status-list-signing-key (Ed25519, HSM)
│
...
| `tir-service@pdtf-platform-prod.iam` | pdtf-platform-prod | `cloudkms.cryptoKeyVersions.useToSign` | `status-list-signing-key` |
...
| Status List Keys | Follows parent | Follows parent | Current status list VC |
...
| Q5 | Should Status List signing keys be separate from adapter keys? | Currently same key. Separate keys add complexity but improve separation of concerns. | Leaning same key |
